...
void menu()                                     
{
    ...
    printf("12. Write record to file\n");
    printf("13. Load record from file\n");
    printf("0. Exit\n");
    printf("==========================================\n");
...
//12. 将记录写入文件
void writeRecord(Student stu[], int n, int m)
{
    ...
    fprintf(fp,"学生人数     课程数量\n");
    fprintf(fp,"%d                %d \n\n",n,m);
    fprintf(fp,"学号     姓名");
    for (int k = 0;k<m; k++) fprintf(fp,"     课程%d", k+1);
    fprintf(fp,"     总分     平均分\n\n");
    for(int i=0;i<n;i++) {
    ...
    fclose(fp);
    printf("记录已保存到 studentrecord.txt\n");
}

//13. 读取文件信息
void loadRecord(Student stu[], int *n, int *m)
{
    FILE*fp = fopen("studentrecord.txt","r");
    if(fp == NULL) 
    {
        printf("无法打开文件！\n");
        return;
    }
    char skip[1024];//创建一个足够大的缓冲区来跳过不需要的数据
    fgets(skip, sizeof(skip), fp);//跳过中文
    fscanf(fp,"%d                %d",n,m);
    fgets(skip, sizeof(skip), fp);// 读完第一行剩余然后换行
    fgets(skip, sizeof(skip), fp);// 跳过空行
    fgets(skip, sizeof(skip), fp);// 跳过表头
    fgets(skip, sizeof(skip), fp);// 跳过空行
    for(int i=0; i<*n ;i++){
        ...
        fscanf(fp,"     %f     %f",&stu[i].total,&stu[i].avg);
    }
    fclose(fp);
    printf("读取成功\n");
}
